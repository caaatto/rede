...
## Security Audit (2026-06-11, Runde 17) — Review der Runde-15/16-Änderungen

Adversarialer C/H/M-Review des in Runde 15/16 + Gruppen-Call-Button neu geschriebenen Codes (zwei parallele Review-Agents: Server + v2-Client). Zwei echte HIGH-Bugs gefunden (beide in dieser Session eingeführt), beide GEFIXT + per Rede.Core-Integrationstest revalidiert. Ein MEDIUM als akzeptiert dokumentiert.

[x] H1 (Server): **FU4-Remote-Requeue greift im Fan-out-/Sealed-Pfad NICHT** — der lokale `sendToUser(userId, message, excludeDeviceId)`-Wrapper (`index.js:252`) hatte nur 3 Parameter und verschluckte den 4. `requeue`-Parameter, den die FU4-Aufrufstellen (`index.js:1297` MESSAGE-Fan-out, `2343` SEALED-Fan-out) übergeben. Folge: im Federation-Modus wurde bei totem Remote-Socket KEIN `requeue` in die Pub/Sub-Nachricht gepackt → `onCrossWorkerMessage` konnte nicht re-queuen → stiller Datenverlust (genau das, was FU4 verhindern sollte). Der gezielte `redis.sendToDevice`-Pfad war korrekt. GEFIXT: Wrapper auf `(userId, message, excludeDeviceId, requeue)` erweitert + durchgereicht.
[x] H2 (Client v2): **Unbegrenzter `ChunkCount` vom Absender → Fetch-Flood / OOM** — `AttachmentInfo.ChunkCount` ist ein vom Absender kontrolliertes Envelope-Feld (`MessageEnvelope.cs`); `BlobService.FetchAsync` nutzte es ungeprüft als Schleifen-Obergrenze (`for i < att.ChunkCount`). Ein bösartiger Kontakt/Gruppen-/Place-Member konnte `chunks` = z.B. 2 Mio. setzen → beim automatischen Bild-Fetch Millionen `BLOB_FETCH`-Roundtrips + unbegrenztes `MemoryStream`-Wachstum (kein Größencheck vor `SecretBox.Open`). GEFIXT: `MaxChunkCount = 24` (ein legitimer 8-MB-Blob braucht ≈18), früher Reject vor dem ersten Fetch + Hard-Cap `MaxCipherReassembleSize` (MaxBlobSize+4096) während des Reassembly. Single-Chunk-Pfad unverändert.
- M1 (Client v2, AKZEPTIERT): **Owner-Transfer-Konsistenzdrift** — nach `PLACE_TRANSFER` ändert der Server `creator_id` sofort, Clients lernen den neuen Owner aber via E2EE-Metadata-DM; ein Member, der dieses DM verpasst, behält lokal den alten Owner als autoritativ und würde Owner-Updates des NEUEN Owners ablehnen → divergierende Owner-Sicht. KEIN Privilege-Escalation (der alte Owner war legitim). Mitigiert: das Transfer-DM läuft über die zuverlässige Pending-Queue (Runde-16-FU4-gehärtet, peek→send→delete), wird also bei Reconnect zugestellt; nur ein nie-wieder-online Member bleibt stale — was für JEDE Metadata-Änderung gilt (inhärent im E2EE-Metadata-Verteilmodell, vgl. "Metadata Bandwidth Amplification"). Sauberer Fix bräuchte ein Metadata-Versionierungs-/Sync-Protokoll (architekturell) — aufgeschoben.

### Bestätigt sauber (Runde-17-Review)
Token-Epoching (HKDF, timingSafeEqual, ≤2h), deliverPending peek→delete (kein Doppel-Send/Verlust, pg `ANY($1::bigint[])` injection-safe), PLACE_TRANSFER-Autorisierung (nur aktueller Owner, Self-Transfer blockiert, Member-Check), XFF-letzter-Hop, Call-Partei-Checks + Feldcaps (vor State-Mutation), OTPK-Reservierung (synchron nach dem einzigen await → atomar pro Tick, kein Doppelvergabe), gcall-callId-Redis-NX (Race-frei), PlaceService/GroupService AcceptSenderKey/AcceptGroupMembers (Membership + Ed25519 + Union-Merge-only), ChatService-TOFU (kein Contact-Pinning für Unbekannte, kein Profil an Unbekannte, OTPK-Konsum nur nach Decrypt), ACK-FIFO (idempotent, kein Versatz), DeriveSFrameKey (hex-only gegen HKDF-Separator-Injection, fail-closed).
...
## OFFENE Follow-ups (Stand 2026-06-09, nach Runde 14) — ALLE GEFIXT in Runde 16 (2026-06-11)

Alle sechs FU + die Runde-15-Lows L1–L6 wurden am 2026-06-11 umgesetzt (siehe "Follow-up-Runde 16" unten). Bestätigt per Build (0 Fehler) + E2E-Tests gegen lokalen Testserver (v2-Integrationstest inkl. 3-Chunk-Attachment; v1 X3DH-DM, Gruppe, kick, rekey).

[x] FU1: **Attachment-Chunking für > 700 KB** — GEFIXT. `BlobService`: MaxBlobSize 8 MB; Ciphertext wird in ≤480-KB-Chunks zerlegt und unter `{blobId}.{i}` hochgeladen (`ChunkCount` in `AttachmentInfo`/Envelope), Fetch fügt vor `SecretBox.Open` wieder zusammen. Single-Chunk-Pfad bleibt wire-kompatibel (bare blobId, ChunkCount 0/1). Server unverändert (jeder Chunk ist ein normaler ≤700-KB-Blob). Test: 1,3-MB-Datei → ChunkCount=3, Roundtrip ✓.
[x] FU2: **Automatischer, owner-gebundener Metadata-Rekey bei Kick/Ban** — GEFIXT. `PlaceService.RotateMetadataKeyAfterRemoval`: wenn der OWNER kickt/bannt, wird der MetadataKey rotiert + alle Channel-Sender-Chains zurückgesetzt + Metadata an verbleibende Member verteilt (R14-H3 akzeptiert Owner-Rotation). Gekickte verlieren SFrame-Ableitung, Metadata-Lesen und künftigen Text. Admin-Kicks verteilen nur wie bisher (Owner-Rotation würde sowieso abgelehnt).
[x] FU3: **Delivery-Token-Epoching** — GEFIXT. Token an 1h-Epoche gebunden, per-Epoche-HMAC-Schlüssel via HKDF aus dem (täglich rotierten) Basis-Secret; nur aktuelle + vorherige Epoche verifizieren → effektive Lebensdauer ≤2h statt 24h. Client unverändert (Token opak, Refresh bei jedem AUTH_OK).
[x] FU4: **Federation Cross-Node-Delivery-Edge-Cases** — GEFIXT. (a) `deliverPending` macht jetzt peek→send→delete-by-id (löscht erst nach bestätigtem `ws.send`-Callback) statt delete-on-fetch. (b) `sendToUser`/`sendToDevice` reichen optionale `requeue`-Info mit; landet ein Frame auf einem Remote-Worker, dessen lokaler Socket inzwischen tot ist, re-queued dieser Worker die Nachricht (in `onCrossWorkerMessage`) statt sie stumm zu verlieren — der optimistische `sent=true` ist damit abgesichert.
[x] FU5: **Place-Ownership-Transfer** — GEFIXT. Neue `PLACE_TRANSFER`/`PLACE_TRANSFER_OK` (beide protocol.js-Kopien + C#). Server: nur aktueller Owner, `setPlaceCreator` aktualisiert `creator_id` (neuer Owner implizit Admin, alter Owner → Admin), in beiden Stores. v2 `PlaceService.TransferOwnership` setzt CreatorId in E2EE-Metadata (R14-H3-akzeptiert) + verteilt. UI-Befehl `/ptransfer <place> <user>`.
[x] FU6: **XFF-Vertrauensgrenze** — GEFIXT. Server liest hinter Trusted Proxy jetzt den LETZTEN `X-Forwarded-For`-Eintrag (der vom Proxy selbst angehängte Hop) statt des ersten (client-kontrollierbar). nginx-Snippet `server/nginx-xff-snippet.conf` (XFF überschreiben statt appenden) — Anwendung durch User (sudo nginx reload).
...
## Funktions-Audit (2026-06-10, Runde 15) — Findings

Funktions-Audit + E2E-Ablauf-Tests (isolierter Testserver, gescriptete v1-CLI-Flows: Register/Invite, Add, 1:1 X3DH beidseitig, Sealed Sender, Gruppen, Device-Link). Happy-Path 1:1 (erst gegenseitig adden, dann senden), Sealed-Zustellung+Decrypt und Device-Linking funktionieren end-to-end; alle Runde-14-Invarianten in v2 und Server bestätigt. Die folgenden Punkte sind echte Funktionsfehler, die im Test bzw. Code-Audit gefunden wurden.

### Critical (Client v2)
[x] K1: **BlobService nullt den Attachment-Key vor dem Senden** — `BlobService.cs:112-125`: `Key = key` ist eine Referenz-Zuweisung (Auto-Property `byte[]`), das anschließende `CryptographicOperations.ZeroMemory(key)` nullt genau das Array, das `att.Key` referenziert (der Kommentar „AttachmentInfo now owns its own copy" ist falsch). `MessageEnvelope.Encode` liest `att.Key` erst beim Senden → Empfänger bekommt 32 Null-Bytes als Key und kann den Blob nie entschlüsseln; der Sender selbst nach Restart ebenfalls nicht (genullter Key steht in der persistierten History). Seit Runde 11 (v2.18.4) im Code. Fix: `Key = (byte[])key.Clone()`.

### High (Client v1, im Ablauf-Test live reproduziert)
[x] H1: **Gruppen-Messaging funktional kaputt — `group.members` wird clientseitig nie gepflegt.** Creator hat `[self]`, Invitee bekommt `[]`; kein Handler/Push aktualisiert die Liste. Folge (live): Sender-Key-Verteilung (Loop über `group.members`) erreicht niemanden, und der H2-Membership-Check lehnt eingehende Sender Keys in beide Richtungen ab → Empfänger sieht stumm gar nichts. Betraf TUI + CLI — und in größerem Umfang auch v2 (siehe H1b). GEFIXT (wire-kompatibel v1↔v2): `groupkey`-Ctrl trägt jetzt optional `members`+`membersSig` (Ed25519 über `GROUPMEMBERS:{groupId}:{sortedMembers}`, alte Clients ignorieren die Felder); neue `groupmembers`-Ctrl-Message (signiert, nur Union-Merge — Entfernen läuft weiterhin über GROUP_KICK) informiert Bestandsmitglieder über neue Member und triggert Redistribution des eigenen Sender Keys; ginvite/InviteToGroup pflegt die lokale Liste und broadcastet; GROUP_KICK_OK räumt members + Sender-Key-State auf. Live-Test: carol↔dave Gruppen-Messaging in beide Richtungen ✓.
[x] H1b: **v2 hatte überhaupt KEINE Sender-Key-Verteilung** — `GroupService`/`PlaceService` erwarten `members[from].chainKey` im Sender-Key-State, aber kein Codepfad hat je einen `senderkey`-Ctrl gesendet oder empfangen (TryHandleControlMessage kannte nur groupkey/profile/placekey/reaction) → Gruppen- UND Place-Textnachrichten zwischen zwei v2-Clients waren nie entschlüsselbar (stiller `else return`). Zusätzlich überschrieben `SendGroupMessage`/`SendChannelMessage` beim Speichern den State mit `{own}` und LÖSCHTEN alle empfangenen Member-Keys. GEFIXT: senderkey-Ctrl senden/empfangen im v1-Wire-Format (Kontext = groupId bzw. `place:{placeId}:{channelId}`), Lazy-Distribution mit `distributedTo`-Tracking VOR dem Ratchet-Advance, State-Erhalt beim Save, Membership+Signatur-Checks beim Annehmen, `HandleGroupInvite`/`AddGroupAsync` überschreiben existierende Gruppen nicht mehr (Order-Race Server-Invite vs. groupkey-DM).
[x] H2: **CLI `send` hängt nach Session-Aufbau endlos** — `cmdSend` wartet nur auf `MESSAGE_ACK` (cli.js:420-430), der Server beantwortet Sealed-Sends aber mit `SEALED_MESSAGE_ACK`. Da Sealed der Normalfall bei etablierter Session ist: kein „Sent", kein Disconnect, und die eigene Nachricht wird nie in die History gespeichert. 4× live reproduziert.
[x] H3: **X3DH-Bootstrap-Deadlock bei unbekanntem Absender** — TUI (index.js:590-594) und CLI (cli.js listen) droppten die X3DH-Initialnachricht eines noch-nicht-geaddeten Absenders nach dem „unknown user"-Hinweis, OHNE die Session zu etablieren; **v2 `ReceiveRatcheted` droppte sogar komplett still**. Da der Server pending Messages beim Abruf löscht, war die Initialnachricht weg; alle Folgenachrichten liefen dauerhaft auf „decrypt failed" — ohne Recovery und ohne Feedback an den Sender. Live reproduziert. GEFIXT in TUI+CLI+v2: X3DH von unbekannten Absendern wird per TOFU verarbeitet (Session etabliert, Nachricht gespeichert, Add-Hinweis; Identitäts-Verify greift weiterhin sobald der Kontakt existiert), Control-Messages von unbekannten Absendern werden ignoriert (kein verifizierter Signing-Key), v2 sendet sein Profil NICHT an unbekannte Absender. Live-Test eve→frank: Erst- und Folgenachricht entschlüsselt, History nach `add` vollständig ✓. Dazu: TUI `pendingOutgoing` Single-Slot → Queue (L6-Teilfix; /ginvite-groupkey+senderkey gehen nicht mehr verloren), Ctrl-Nachrichten landen nicht mehr in der Chat-History des PREKEY_BUNDLE-Pfads.

### Medium (Client v2)
[x] M1: **Doppelte Nachricht bei Partial-Session-Kontakt** — `ChatService.cs:339-370 + 1105-1119`: Hat ein Kontakt Devices mit UND ohne Session, wird sofort gesendet+persistiert UND in `_pendingOutgoing` eingereiht; `HandlePrekeyBundle` persistiert nach Bundle-Ankunft ein ZWEITES `ChatMessage` → doppelte Bubble, `OnMessageSent` feuert zweimal. GEFIXT: `_pendingOutgoing`-Tupel trägt jetzt die bereits persistierte `ChatMessage`-Referenz; der Bundle-Pfad etabliert nur die restlichen Sessions, nutzt dieselbe Instanz für die ACK-FIFO-Slots und persistiert nicht erneut.
[x] M2: **Silent-Drop bei leerer Devices-Map ohne Legacy-State** — `ChatService.cs:304-371`: Bei `contact.Devices.Count == 0` und fehlendem Legacy-Ratchet blieb `needBundle` leer → kein `FETCH_PREKEY_BUNDLE`, kein `OnSystemMessage`, die Nachricht verschwand kommentarlos. GEFIXT: In dem Fall wird jetzt das Pre-Key-Bundle gefetcht (Sentinel-Eintrag in `needBundle`) — Geräte werden discovered, Nachricht geht nach Session-Aufbau raus.
[x] M3: **Own-Echo ohne Device-Unterscheidung (Multi-Device)** — `GroupService.cs`, `PlaceService.cs`: Der `from == Profile.UserId`-Pfad prüfte kein `fromDeviceId` — das Echo einer vom EIGENEN Zweitgerät gesendeten Group-/Place-Nachricht dequeued einen fremden FIFO-Slot (msgId-Versatz). GEFIXT: Echo mit fremder eigener DeviceId wird ignoriert (kein Dequeue). Anzeige solcher Nachrichten bleibt offen — Sender Keys werden nie an eigene Geschwister-Geräte verteilt (Multi-Device-Designlücke, gilt auch für v1; als Follow-up vermerkt).

### Medium (Server)
[x] M4: **OTPK-Vergabe-Race bei konkurrierenden Erstkontakten** — `fetchPreKeyBundle` lieferte immer `otpks[0]`; die Reservierung machte nur den Re-Fetch DESSELBEN Senders idempotent. Zwei nahezu gleichzeitige Erstkontakte zum selben Ziel bekamen dieselbe klassische + PQ-OTPK — der zweite lief beim Empfänger auf „X3DH key agreement failed". GEFIXT: Beide Stores liefern jetzt zusätzlich die vollen OTPK-Listen (`oneTimePreKeysAll`/`pqOneTimePreKeysAll`, gehen NICHT auf die Wire); der Handler wählt pro Sender den ersten OTPK, der nicht für einen anderen Sender reserviert ist (klassisch UND PQ), und die Reservierung deckt jetzt auch den PQ-Key ab. Einschränkung: Reservierungen bleiben per-Node-in-memory — über die Federation hinweg ist das Race nur verkleinert, nicht eliminiert (Redis-Reservierung wäre der Vollausbau).
[x] M5: **`SESSION_END` broadcastet bedingungslos „offline"** — index.js; der `close`-Handler prüft korrekt cross-node `isUserOnline`. Meldete sich EIN Gerät eines Multi-Device-Users sauber ab, erschien der User allen als offline, obwohl ein zweites Gerät online war. GEFIXT: SESSION_END prüft jetzt dieselbe `isUserOnline`-Bedingung wie der close-Handler.

### High (Server, Federation)
[x] H4: **Gruppen-Calls brechen über Node-Grenzen — per-Node `callId` → inkompatible SFrame-Keys.** `activeGroupCalls` war eine lokale Map; `handleGCallRequestToken` erzeugte pro Node eine eigene `callId`, die der Client in die SFrame-HKDF mischt. LB-Round-Robin → verschiedene callIds für denselben LiveKit-Raum → gegenseitig nicht entschlüsselbar. Verschärft durch den 60s-Cleanup, der Teilnehmer gegen die LOKALE `clients`-Map prüfte: remote Teilnehmer wurden als Phantome gepurgt und die callId mitten im Call rotiert. GEFIXT: Gruppen-Call-Session (scope, callId, participants) liegt jetzt in Redis (`gcall:{room}`, 4h TTL, atomares `SET NX` gegen Create-Races — Verlierer übernimmt die callId des Gewinners; in-memory Fallback für Single-Node); Cleanup purgt Teilnehmer nur noch, wenn sie GLOBAL offline sind (`isUserOnline`), leere Sessions erst nach dem 10-min-Stale-Fenster. Lokale Map bleibt nur als Count-/Cleanup-Mirror. Beim Gruppen-Call-Integrationstest über die Federation gezielt verifizieren (zwei Clients, einer pro Node).

### Low / Niedrig
[x] L1 (Server): GEFIXT — `CALL_HANGUP`/`CALL_REJECT`/`CALL_MUTE` prüfen jetzt sender ∈ {callerId, calleeId} vor State-Delete/Relay; `to` via `validateInternalId`; Feldgrößen-Caps für sdp/candidate/srtpParams/reason/mode. Ursprünglich: löschten Call-State ohne Partei-Check (index.js:2804-2808), `CALL_MUTE` analog (2865-2882) — callIds sind nur Teilnehmern bekannt, daher kaum ausnutzbar; Konsistenz-Härtung. Call-Signaling-Relay (`sdp`/`candidate`/`srtpParams`) ohne Feldgrößen-Cap bis zur 1-MB-Framegrenze.
[x] L2 (Server): GEFIXT — `storePreKeyBundle` dedupliziert OTPKs jetzt per Key-String (Map) statt per Objekt-Set. Ursprünglich: v1-OTPK-Dedup defekt — `storePreKeyBundle` dedupliziert klassische OTPKs per `Set` über `{id,key}`-OBJEKTE (Objektidentität!) → Re-Uploads akkumulieren Duplikate, `slice(-200)` verdrängt noch gültige Keys. PQ-Pfad macht es richtig (Map per Key-String). Nur v1-Clients betroffen.
[x] L3 (Server): GEFIXT — `cleanupOnExit` ist async und awaited `zeroMasterKey`, Exit via `.finally`. Ursprünglich: awaitete `store.zeroMasterKey()` nicht und exitet sofort — im pg-Backend laufen `pool.end()`/`fill(0)` nie. Hygiene.
[x] L4 (v2): GEFIXT — bei vollem `_pendingAck` wird der ÄLTESTE Slot dequeued + geloggt (Chat/Group/Place). Ursprünglich: droppte bei vollem `_pendingAck` den NEUEN Slot statt des ältesten, ohne Log (ChatService.cs:425 analog Group/Place) — Versatz bis zum nächsten Reconnect statt sauberem Drop.
[x] L5 (v1): GEFIXT — TUI REGISTER_OK löscht Temp-Profil + validiert serverSigningKey-Format; CLI bekam `confirm`/`kick`/`rekey`-Kommandos, DEVICE_ADDED-Handler, per-Device-Sender-Key-Verteilung, Ratchet-Rollback bei Send-Fehler, ERROR-Exit für group-new/gen-link. Ursprünglich: TUI REGISTER_OK löschte Temp-Profil nicht (index.js:485-507, cli.js tut es) und pinnt `serverSigningKey` ohne Formatvalidierung (M1-Fix dort unvollständig). CLI: kein `confirm`/`kick`/`rekey`-Kommando, kein `DEVICE_ADDED`-Handler, Sender-Key-Verteilung ignoriert per-Device-Sessions, kein Ratchet-Rollback bei Send-Fehler, `group-new`/`gen-link` hängen bei Server-ERROR statt zu exiten.
[x] L6 (v1): GEFIXT — TUI `pendingOutgoing` ist jetzt eine Queue (ginvite-groupkey+senderkey gehen nicht mehr verloren); `padMessage` wirft bei >16382 Bytes, Sender prüfen ~16 KB vorab (TUI+CLI). Ursprünglich: Single-Slot (index.js:150/305) — `/ginvite` ohne bestehende Session überschreibt den groupkey mit dem senderkey → Invitee kann nie beitreten. `padMessage` ohne Größen-Cap: 16383–65535 Bytes → stille Truncation/Garbage, >65535 → ungefangene RangeError (TUI-Crash).

### Info / Doku-Drift (kein Fix nötig, zur Kenntnis)
- DR `MAX_SKIP` ist real 256 (crypto.js:388), nicht 1000 — die 1000 gilt nur für Sender-Key-Decrypt. Memory/Doku ungenau, Code in beiden v1-Clients konsistent.
- `KEY_EXCHANGE`/`GROUP_MEMBERS`/`STATUS_SUBSCRIBE` sind tote Protokoll-Einträge ohne Server-Handler; `GCALL_ACTIVE`/`CALL_PARTICIPANTS` werden nie gesendet (Doku deklariert sie als aktiv).
- LiveKit-Logik lebt inline in `index.js:2892-3136`, nicht in `server/livekit/` (dort nur Deployment-README).
- In `handleMessage` wird die Nonce vor den Header-Validierungen verbraucht — ein wegen Oversize abgelehnter Retry mit identischer Nonce ergäbe „Duplicate nonce". Harmlos (Clients generieren pro Send neu).
- FU4 präzisiert: `deliverPending` sendet bis zu 100+100 Pending-Messages in EINEM WS-Frame, dessen Inhalt zu dem Zeitpunkt bereits aus der DB gelöscht ist. Im Ablauf-Test lokal demonstriert: Client-Kill während der Verarbeitung = Nachricht endgültig weg (kein Federation-Szenario nötig).
...
### Analysiert, kein Fix nötig
- `ProfileStore.ChangePassphraseAsync` verifiziert selber nicht, aber Caller (`OnChangePassphraseRequested`-Handler in `MainWindow.axaml.cs`) prüft via `CryptographicOperations.FixedTimeEquals` gegen `_auth.Passphrase`. FIDO2-PMS überlebt den Wechsel (`_activePms` lebt in `ProfileStore` und wird in jede Key-Derivation gemischt). OK — re-verifiziert 2026-06-10.
