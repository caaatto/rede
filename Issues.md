...
### Low / Niedrig
[ ] L1 (Server): `CALL_HANGUP`/`CALL_REJECT` löschen Call-State ohne Partei-Check (index.js:2804-2808), `CALL_MUTE` analog (2865-2882) — callIds sind nur Teilnehmern bekannt, daher kaum ausnutzbar; Konsistenz-Härtung. Call-Signaling-Relay (`sdp`/`candidate`/`srtpParams`) ohne Feldgrößen-Cap bis zur 1-MB-Framegrenze.
[ ] L2 (Server): v1-OTPK-Dedup defekt — `storePreKeyBundle` dedupliziert klassische OTPKs per `Set` über `{id,key}`-OBJEKTE (Objektidentität!) → Re-Uploads akkumulieren Duplikate, `slice(-200)` verdrängt noch gültige Keys. PQ-Pfad macht es richtig (Map per Key-String). Nur v1-Clients betroffen.
[ ] L3 (Server): `cleanupOnExit` awaited `store.zeroMasterKey()` nicht und exitet sofort — im pg-Backend laufen `pool.end()`/`fill(0)` nie. Hygiene.
[ ] L4 (v2): FIFO-Cap droppt bei vollem `_pendingAck` den NEUEN Slot statt des ältesten, ohne Log (ChatService.cs:425 analog Group/Place) — Versatz bis zum nächsten Reconnect statt sauberem Drop.
[ ] L5 (v1): TUI `REGISTER_OK` löscht das Temp-Profil nicht (index.js:485-507, cli.js tut es) und pinnt `serverSigningKey` ohne Formatvalidierung (M1-Fix dort unvollständig). CLI: kein `confirm`/`kick`/`rekey`-Kommando, kein `DEVICE_ADDED`-Handler, Sender-Key-Verteilung ignoriert per-Device-Sessions, kein Ratchet-Rollback bei Send-Fehler, `group-new`/`gen-link` hängen bei Server-ERROR statt zu exiten.
[ ] L6 (v1): TUI `pendingOutgoing` ist Single-Slot (index.js:150/305) — `/ginvite` ohne bestehende Session überschreibt den groupkey mit dem senderkey → Invitee kann nie beitreten. `padMessage` ohne Größen-Cap: 16383–65535 Bytes → stille Truncation/Garbage, >65535 → ungefangene RangeError (TUI-Crash).
