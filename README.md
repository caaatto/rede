...
Rede supports end-to-end encrypted voice calls. Audio is encrypted with
SRTP (AES-128-CM + HMAC-SHA1-80). The server relays encrypted binary
WebSocket frames between call peers without ever decrypting the audio.

The call transport mode is derived from your connection:

| Transport | Latency | IP visible to server | Audio encryption |
|---|---|---|---|
| Direct (WSS) | ~50-100ms | Yes | SRTP (E2EE) |
| I2P | ~500-2000ms | No (garlic routed) | SRTP (E2EE) |
| Tor | ~300-1000ms | No (onion routed) | SRTP (E2EE) |

There is no per-call mode selection — if you connect over I2P, your
calls are anonymous. If you connect direct, calls go direct.
...
duration timer, and a mode indicator icon.
...
- Voice transport: derived from connection (I2P/Tor = anonymous, Direct = low latency)
